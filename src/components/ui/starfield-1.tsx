...
const Starfield: React.FC<StarfieldProps> = ({
    ...
    // Fix: Combine prop and state to allow both controlled and interactive hyperspace
    ...
    const resize = () => {
        ...
        measureViewport();
        const canvas = canvasRef.current;
        sd.current.cw = canvas ? canvas.width : 0;
        sd.current.ch = canvas ? canvas.height : 0;

        if (canvas && sd.current.ctx && (sd.current.cw !== sd.current.w || sd.current.ch !== sd.current.h)) {
            sd.current.x = Math.round(sd.current.w / 2);
            sd.current.y = Math.round(sd.current.h / 2);
            sd.current.z = (sd.current.w + sd.current.h) / 2;
            sd.current.star.colorRatio = 1 / sd.current.z;
